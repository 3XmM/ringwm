...
func main() {
	...
	wm.grabKey(111, xproto.ModMask4)
	wm.grabKey(116, xproto.ModMask4)
...
func (wm *WindowManager) arrangeWindows() {
	...
	effHeight := int(wm.ScreenHeight) - wm.BarHeight
	...
	if wm.RingMode {
		if wm.ZoomMode {
			...
			zh := effHeight - 2*wm.Gaps
			zx := wm.Gaps
			zy := wm.Gaps + wm.BarHeight
		...
		radiusY := int(float64(effHeight) * 0.22)
		...
		for i, win := range wins {
			...
			wh := int(float64(effHeight) * 0.45 * scale)
			x := int(float64(cx)+float64(radiusX)*math.Cos(angle)) - ww/2
			y := int(float64(cy)+float64(radiusY)*math.Sin(angle)) - wh/2 + wm.BarHeight
		...
		for i, win := range wins {
			...
			case 1:
				_ = xproto.ConfigureWindowChecked(wm.X, win,
					xproto.ConfigWindowX|xproto.ConfigWindowY|xproto.ConfigWindowWidth|xproto.ConfigWindowHeight,
					[]uint32{0, uint32(wm.BarHeight), uint32(sw), uint32(sh)}).Check()
			case 2:
				if i == 0 {
					_ = xproto.ConfigureWindowChecked(wm.X, win,
						xproto.ConfigWindowX|xproto.ConfigWindowY|xproto.ConfigWindowWidth|xproto.ConfigWindowHeight,
						[]uint32{0, uint32(wm.BarHeight), uint32(sw / 2), uint32(sh)}).Check()
				} else {
					_ = xproto.ConfigureWindowChecked(wm.X, win,
						xproto.ConfigWindowX|xproto.ConfigWindowY|xproto.ConfigWindowWidth|xproto.ConfigWindowHeight,
						[]uint32{uint32(sw / 2), uint32(wm.BarHeight), uint32(sw / 2), uint32(sh)}).Check()
				}
			case 3:
				if i == 0 {
					_ = xproto.ConfigureWindowChecked(wm.X, win,
					...
					_ = xproto.ConfigureWindowChecked(wm.X, win,
						xproto.ConfigWindowX|xproto.ConfigWindowY|xproto.ConfigWindowWidth|xproto.ConfigWindowHeight,
						[]uint32{uint32(sw / 2), uint32(wm.BarHeight), uint32(sw / 2), uint32(sh / 2)}).Check()
				} else {
					_ = xproto.ConfigureWindowChecked(wm.X, win,
						xproto.ConfigWindowX|xproto.ConfigWindowY|xproto.ConfigWindowWidth|xproto.ConfigWindowHeight,
						[]uint32{uint32(sw / 2), uint32(wm.BarHeight + sh/2), uint32(sw / 2), uint32(sh / 2)}).Check()
				}
			default:
				cols := 2
				rows := (cnt + 1) / 2
				...
				_ = xproto.ConfigureWindowChecked(wm.X, win,
					xproto.ConfigWindowX|xproto.ConfigWindowY|xproto.ConfigWindowWidth|xproto.ConfigWindowHeight,
					[]uint32{uint32(col * colW), uint32(wm.BarHeight + row*rowH), uint32(colW), uint32(rowH)}).Check()
			}
		}
	}
	wm.drawBar()
...
func (wm *WindowManager) drawBar() {
	gcid, _ := xproto.NewGcontextId(wm.X)
	defer xproto.FreeGcontext(wm.X, gcid)
	_ = xproto.CreateGcChecked(wm.X, gcid, wm.Root,
		xproto.GcForeground|xproto.GcBackground|xproto.GcFunction,
		[]uint32{0x000000, 0xFFFFFF, xproto.GxCopy}).Check()
	_ = xproto.PolyFillRectangleChecked(wm.X, wm.Root, gcid, []xproto.Rectangle{{
		X: 0, Y: 0, Width: wm.ScreenWidth, Height: uint16(wm.BarHeight),
	}}).Check()
	_ = xproto.ChangeGcChecked(wm.X, gcid, xproto.GcForeground, []uint32{0xFFFFFF}).Check()
	leftText := fmt.Sprintf(" RingWM 1.0 | WS: %d ", wm.CurrentWorkspace+1)
	_ = xproto.ImageText8Checked(wm.X, xproto.Drawable(wm.Root), gcid, 5, int16(wm.BarHeight-5), leftText).Check()
	var rightText string
	if wm.RingMode {
		if wm.ZoomMode {
			rightText = "[ FULLSCREEN MODE ] "
		} else {
			rightText = "[ RING MODE ] "
		}
	}
	if rightText != "" {
		rightX := int(wm.ScreenWidth) - (len(rightText) * 8) - 10
		if rightX > len(leftText)*8+10 {
			_ = xproto.ImageText8Checked(wm.X, xproto.Drawable(wm.Root), gcid, int16(rightX), int16(wm.BarHeight-5), rightText).Check()
		}
	}
}
