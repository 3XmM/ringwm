...
func (wm *WindowManager) drawBar() {
	...
	vals := []uint32{0x000000, 0xFFFFFF}
	_ = xproto.CreateGc(wm.X, gcid, xproto.Drawable(wm.Root), mask, vals)

	_ = xproto.PolyFillRectangle(wm.X, xproto.Drawable(wm.Root), gcid, []xproto.Rectangle{{
		X: 0, Y: 0, Width: wm.ScreenWidth, Height: uint16(wm.BarHeight),
	}})

	_ = xproto.ChangeGc(wm.X, gcid, xproto.GcForeground, []uint32{0xFFFFFF})

	leftText := fmt.Sprintf(" RingWM 1.0 | WS: %d ", wm.CurrentWorkspace+1)
	_ = xproto.ImageText8(wm.X, xproto.Drawable(wm.Root), gcid, 5, int16(wm.BarHeight-5), leftText)
	...
	if rightText != "" {
		...
		if rightX > len(leftText)*8+10 {
			_ = xproto.ImageText8(wm.X, xproto.Drawable(wm.Root), gcid, int16(rightX), int16(wm.BarHeight-5), rightText)
		}
	}
}
